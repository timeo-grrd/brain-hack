...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function resolveApiBaseUrl() {
    ...
    // If frontend is served on another local port (e.g. Live Server), target Node API on 3000.
  ...
  bouton.addEventListener("click", async function () {
    ...
    try {
      ...
      if (!response.ok) {
        const fallback = rawBody && !rawBody.startsWith("<") ? rawBody : "Erreur API inconnue";

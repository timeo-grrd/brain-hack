...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function getHeaderAccountPath() {
        ...
        const onApiIaPage = normalizedPath.includes('/api_ia/');
    ...
    function getReadArticleIds() {
        const connectedUser = getConnectedUser();
        if (!connectedUser || !window.BrainHackProgress) {
            return new Set();
        }
    ...
    function updateArticleReadBadges() {
        ...
        TRACKED_ARTICLE_IDS.forEach(articleId => {
            ...
            applyCardReadBadge(card, readSet.has(articleId));
        });
    }

    function trackCardRead(card) {
        if (!card || !window.BrainHackProgress) {
            return;
        }
    ...
    if (featuredIaCard) {
        ...
        featuredIaCard.addEventListener('click', function () {
            trackCardRead(featuredIaCard);
            openIaArticleModal();
        });
